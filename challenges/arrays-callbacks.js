...
with only the animal_name and scientific_name of each animal. displayNames will be an array of strings, and each string should follow this 
pattern: "Name: Jackal, asiatic, Scientific: Canis aureus."

*/


let displayNames = zooAnimals.map(function(item){
...
const lowPopulationAnimals = zooAnimals.filter(function(item){
  ...
  const lowPopAnimals = lowPopulationAnimals.map(function(item){
...
The zoos need to know their total animal population across the United States. Find the total population from all the zoos
 using the .reduce() method. Remember the reduce method takes two arguments: 
 a callback (which itself takes two args), and an initial value for the count.

*/
let populationTotal = zooAnimals.reduce(function (accum, item, index) {
